...
function SolutionTable ({ headers, data, remove, edit }) {
              ...
              <tbody>
                {data.map((item, index) => (
                  item && (
                    ...
                    >
                      <>
                        <td className="whitespace-nowrap px-6 py-4" key={`${item.solutionId}-key`}>{item.solutionId}</td>
                        <td className="whitespace-nowrap px-6 py-4" key={`${item.description}-key`}>{item.description}</td>
                        <td className="whitespace-nowrap px-6 py-4" key='item.actions-key'>
                          <button className="text-gray-500 hover:text-gray-700 ml-1" onClick={() => handleEdit(item.solutionId) }>
                            <IoPencilSharp size={20} />
                          </button>
                          <button className="text-red-500 hover:text-red-700" onClick={() => { handleOpenModal(item.solutionId) }}>
                            <IoTrash size={20} />
                          </button>
                        </td>
                      </>
